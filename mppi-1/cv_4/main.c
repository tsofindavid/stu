...
void main(void)
{
	...
	P1REN = 0b10110110; //B6, ak bolo len 1 tl na P1.3  P1 rezistor enable
	...
	P1IES = BIT3;	//reaguj na zostupnu hranu iba na P1.3, tam je tlacitko
	...
	//P1IE = BIT3;	//povol spustenie ISR len od pinu P1.3
	                //vsetky bity okrem 3 bitu
	P1IE = BIT1|BIT2; //povol psustenie ISR od pinov P1.1 do P1.2
	P1IE |= BIT3; //este povol enkoder
	
	_BIS_SR(GIE);	//povol vsetky maskovatelne prerusenia
					//od tohto okamihu je mozne spustit ISR

	//spotreba:
	//ked slucka while bezi tak 371uA (main)
	//ked stlacim tak je odber 386uA (ISR)
	//
	//+po odporejni komunikatora 0uA
	...
	//spotreba:
	//ked slucka while bezi tak 0.2uA (main)
	//ked stlacim tak je odber 388uA (ISR)

    //+po odporejni komunikatora 0uA

	//obidve tieto instrukcie su tie iste (len ine poradie hodnot)
...
*/
    ...
    if (P1IFG&BIT3){    //
        if(P1IN&BIT4)   //cita bit4 kde je druhy signal enkoderu
        {
    ...
    //spotrebu ovplyvnuju plavajuce piny (treba ich osetrit)
    //na  2 piny mozeme pripojit 1 enkoder
